...
export class GestionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      this.grafica();
      this.totalVenta = this.precioVenta.reduce(
        (total, numero) => total + numero,
        0
      );
      this.totalProducto = this.precioProducto.reduce(
        (total, numero) => total + numero,
        0
      );
      this.totalInsumo = this.precioInsumo.reduce(
        (total, numero) => total + numero,
        0
      );
      console.log("siii");
  ...
  sumarArrays(array1: number[], array2: number[]): number[] {
    ...
    const arrayResultado = array1.map(
      (valor, indice) => valor + array2[indice]
    );
  ...
  exportExcel() {
    this.gestionService.excelUsuario().subscribe((x) => {
      const blob = new Blob([x], {
        type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      });

      const fechaDescarga = new Date().toLocaleDateString().replace(/\//g, "-");
      const nombreArchivo = `inventario_${fechaDescarga}.xlsx`;
